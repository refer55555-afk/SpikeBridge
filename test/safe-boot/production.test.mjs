...
test("public tunnel integration ships only a credential-free example",async()=>{
  const example=await readFile(path.join(ROOT,"tunnel","profile.example.yaml"),"utf8");
  assert.match(example,/YOUR_TUNNEL_ID/);
  assert.match(example,/http:\/\/127\.0\.0\.1:7690\/mcp/);
  assert.doesNotMatch(example,/\btunnel_[A-Za-z0-9]{16,}\b/);
  assert.doesNotMatch(example,/\borg-[A-Za-z0-9]{12,}\b/);
});
