...
$exe=Join-Path $operator 'Bridge.Desktop.exe'
$icon=Join-Path $operator 'assets\bridge-desktop.ico'
& $csc /nologo /target:winexe /optimize+ /reference:System.Windows.Forms.dll ('/win32icon:'+$icon) ('/out:'+$exe) (Join-Path $PSScriptRoot 'PanelLauncher.cs')
if($LASTEXITCODE -ne 0){throw 'Bridge 窗口启动器编译失败。'}
$shell=New-Object -ComObject WScript.Shell
$desktop=[Environment]::GetFolderPath('Desktop')
$linkPath=Join-Path $desktop 'Bridge.lnk'
$link=$shell.CreateShortcut($linkPath)
$link.TargetPath=$exe;$link.WorkingDirectory=$root;$link.IconLocation=$icon+',0';$link.Description='Bridge：查看任务、Token、规则、Memory、Bridge A/B 和安全恢复';$link.Save()
